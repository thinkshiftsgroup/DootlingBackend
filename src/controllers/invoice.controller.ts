...
export const getInvoices = asyncHandler(async (req: Request, res: Response) => {
  const storeId = parseInt(req.params.storeId);
  const { search, warehouseId, customerId, status, minAmount, maxAmount, page = "1", limit = "10" } = req.query;
...
export const exportInvoicesCSV = asyncHandler(async (req: Request, res: Response) => {
  const storeId = parseInt(req.params.storeId);
  const invoices = await invoiceService.getAllInvoices(storeId);
  ...
  const data = invoices.map((invoice: any) => ({
    ...
    customer: invoice.customer ? `${invoice.customer.firstName} ${invoice.customer.lastName}` : "",
